...
Для проекта можно создать `.agent/config.json` на основе [agent.config.example.json](agent.config.example.json). В нём допустимо хранить только несекретные настройки: provider, лимиты, память и правила проекта. Пример есть в [тестовом проекте](examples/agent-fixture/.agent/config.json). Поддержка отдельных profiles будет добавлена позднее.

Если в корне workspace есть `AGENTS.md`, агент загружает его перед каждым запуском (`ask`, `inspect`, `run`, `resume`) и передаёт модели как системную инструкцию с более высоким приоритетом, чем пользовательский промпт. Файл необязателен — его отсутствие не влияет на выполнение.
...
- `resume` c `codex-local` продолжает исходный тред Codex (`thread_id` сохраняется в SQLite при первом ходе задачи); для `mock` и `openai-compatible`, а также для задач без сохранённой сессии, используется прежний текстовый fallback на основе последнего checkpoint'а.
